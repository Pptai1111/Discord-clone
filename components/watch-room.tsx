...
export const WatchRoom = ({ chatId, member }: WatchRoomProps) => {
  ...
  useEffect(() => {
    if (socket && userId) {
      ...
      socket.emit('join-room', { roomId: chatId, role: member.role });
      return () => {
      ...
      {!isClientSide ? (
        ...
        <>
          ...
          <div className="flex-1 flex flex-col relative">
            {renderPlayer()}
            {currentVideo && (
              <div className="flex items-center bg-zinc-900 p-2 border-t border-zinc-700 z-20 relative">
                <Button 
                  ...
                  <div 
                    ...
                    onClick={(e) => {
                      if (isAdmin && playerRef.current) {
                        ...
                        const seekTo = duration > 0 ? percent * duration : 0;
                        handleSeek(seekTo);
                      }
                    }}
                  >
