        ...
        if (data.playlist && Array.isArray(data.playlist)) {
          try {
            ...
            if (validPlaylist.length > 0) {
              ...
              // CHỈ xóa playlist nếu local cũng đang rỗng, hoặc xác nhận phòng thực sự không có video
              if (playlist.length > 0) {
                console.warn('[CLIENT] Nhận playlist rỗng nhưng local đang có video, giữ nguyên playlist local!');
                // Không xóa playlist local
              } else {
                setPlaylist([]);
